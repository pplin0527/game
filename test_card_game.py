...
class TestCardGame(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.basic_attack = AttackCard(
            "Attack", 1, 6, "Deal 6 damage to the enemy")
        self.double_attack = AttackCard(
            "Double Attack", 2, 12, "Deal 12 damage to the enemy")
        self.basic_defense = DefenseCard(
            "Defense", 1, 5, "Gain 5 defense points")
        self.double_defense = DefenseCard(
            "Double Defense", 2, 10, "Gain 10 defense points")
        self.draw_card = DrawCard("Draw Card", 1, 1, "Draw 1 additional card")

        self.enemy_attack = AttackCard(
            "Attack", 0, 6, "Deal 6 damage to the player")
        self.enemy_defense = DefenseCard(
            "Defense", 0, 5, "Gain 5 defense points")

        self.player_deck = [self.basic_attack, self.double_attack,
                            self.basic_defense, self.double_defense, self.draw_card]
        self.enemy_deck = [self.enemy_attack, self.enemy_attack,
                           self.enemy_attack, self.enemy_defense]
    ...
    def test_attack_card(self):
        ...
        self.assertEqual(self.enemy.current_hp, initial_hp -
    ...
    def test_defense_card(self):
        ...
        self.assertEqual(self.player.defense_point,
                         initial_defense + self.basic_defense.defense_point)

    def test_draw_card(self):
        initial_hand_size = len(self.player.hand_card)
        self.draw_card.play(self.player)
        self.assertEqual(len(self.player.hand_card),
                         initial_hand_size + self.draw_card.draw_count)
    ...
    def test_character_take_damage(self):
        ...
        self.assertEqual(self.player.current_hp, initial_hp -
                         (damage - initial_defense))
    ...
    def test_diamond_enemy_distracts_spade_enemy(self):
        # Create Diamond and Spade enemies
        diamond_enemy = DiamondEnemy("Diamond", 20)
        spade_enemy = SpadeEnemy("Spade", 20)

        # Check initial state
        self.assertFalse(spade_enemy.skip_turn)

        # Perform DiamondEnemy's special talent
        diamond_enemy.special_talent(spade_enemy)

        # Check if SpadeEnemy is distracted
        self.assertTrue(spade_enemy.skip_turn)

        # Simulate SpadeEnemy's turn (resetting skip_turn to False)
        spade_enemy.skip_turn = False

        # Check if SpadeEnemy is no longer distracted
        self.assertFalse(spade_enemy.skip_turn)
